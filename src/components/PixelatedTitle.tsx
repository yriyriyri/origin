...
export default function PixelatedTitle({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const letterSpacing = parsePx(computed.letterSpacing, fontSize * 0.08);
      ...
      const pixelWidth = Math.max(1, Math.round(canvas.width / (blockSize * dpr)));
  ...
  return (
    <span className="root">
      ...
      <span className="crisp" ref={crispRef} style={{ opacity: showPixelated ? 0 : 1 }}>
        {text}
      </span>
      <canvas
        aria-hidden="true"
